...
export function step28() {
  ...
  function mondai_2_create() {
    ...
    for (let i = 4; i < 8; i++) {
      const bunboValue = Math.floor(Math.random() * 7 + 3);
      const bunshiValue = Math.floor(Math.random() * (bunboValue - 1) + 1);
      const taibunsuValue = Math.floor(Math.random() * 3 + 1);
      const mixedFraction = taibunsuValue * bunboValue + bunshiValue;
      answer_array[i - 2] = `
  ...
  function mondai_3_create() {
    ...
    for (let i = 8; i < 11; i++) {
      ...
      let mixedFraction;
      if (i !== 10) mixedFraction = taibunsuValue * bunboValue + bunshiValue;
      else mixedFraction = taibunsuValue * bunboValue;
